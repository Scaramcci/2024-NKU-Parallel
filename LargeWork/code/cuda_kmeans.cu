                   ...
                   int numIntermediates2)   //  The next power of two
{
	...
	begin_addr = 0;
	end_addr = numIntermediates;
	begin_addr += tid;
	while(begin_addr < end_addr)
	{
		intermediates[tid] += deviceIntermediates[begin_addr];
                   ...
                   int    *loop_iterations)
{
    ...
    const unsigned int numReductionThreads = 512 ;
	//const unsigned int numReductionBlocks = (numClusterBlocks +numReductionThreads-1)/ numReductionThreads;
...
##########0###########
**********************************************************************
CUDA Error 17: invalid device pointer
